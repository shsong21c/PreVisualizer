...
public:
	...
	{
		if(bState!=m_bActivate) {m_bActivate = bState; Invalidate();}
	}
	bool GetButtonState() {return m_bActivate;}

	void SetBkBlink(BOOL bStart, UINT nElapse);
	void SetRepeatAction() {m_bRepeatAction = true;}
	void SetHoverAction(bool bUse) {m_bHoverAction = bUse;}
...
private:
	...
	bool		m_bActivate;		// Activate or not?
	BOOL		m_bBkBlink;			// Blinking
	int			m_nBkBlinkStep;		// Blinking Step
	UINT		m_nTimerId;			// Timer ID
	bool		m_bRepeatAction;	// Repeat or not?
	bool		m_bHoverAction;		//Hover or not?
	UINT		m_nSent;			// Press repeated
