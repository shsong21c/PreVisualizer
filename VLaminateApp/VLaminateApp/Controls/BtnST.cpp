...
//m_bActivate
			Invalidate();// Send notification message only on rising blink
		}
	}
	else
	{
		if(m_bRepeatAction==true)
		{
    		if((GetState() & BST_PUSHED)==0) return;

			SetTimer(IDT_TIMER, REPEAT_DELAY, NULL);
//			CWnd *pWnd = GetParent();
			GetParent()->SendMessage(WM_USER+600, MAKELONG(GetDlgCtrlID(), BN_CLICKED), (LPARAM)m_hWnd);
			m_nSent++;
		}
...
void CButtonST::SetBkBlink(BOOL bStart, UINT nElapse)
{
...
void CButtonST::OnLButtonUp(UINT nFlags, CPoint point)
{
	if(m_bRepeatAction==true)
	{
		...
		{
			ReleaseCapture();
			if(m_nSent == 0 && (GetState() & BST_PUSHED) != 0)
...
void CButtonST::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if(m_bHoverAction==false) return;
...
//		byIndex = (m_csBitmaps[1].hBitmap == NULL ? 0 : 1);
		if(m_bBkBlink==FALSE)
			byIndex = (m_bActivate == false ? 1 : 0);
		else
			byIndex = (m_nBkBlinkStep == 1 ? 1 : 0);
	}
...
DWORD CButtonST::SetURL(LPCTSTR lpszURL)
{
	...
	{
		// Store the URL
		_tcsncpy_s(m_szURL, sizeof(m_szURL), lpszURL, _MAX_PATH);
	} // if
...
DWORD CButtonST::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
	...
	if (lpszSound)
	{
		...
		{
			_tcscpy_s(m_csSounds[byIndex].szSound, 260*sizeof(char), lpszSound);
